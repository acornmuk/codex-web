...
웹의 프로젝트 폴더 선택기는 기본적으로 Docker 컨테이너의 `/`부터 전체
파일시스템을 탐색한다. 호스트 폴더를 사용하려면 먼저 해당 폴더를 컨테이너의
임의 경로로 마운트해야 한다. 탐색 범위를 다시 제한하려면 서버 시작 전에
`CODEX_WEB_WORKSPACE_ROOT`로 허용할 최상위 경로를 지정할 수 있다.

컨테이너의 설정·인증 관련 디렉터리도 선택할 수 있으므로 `codex-web` 서버를
인터넷이나 신뢰할 수 없는 네트워크에 공개하면 안 된다.
